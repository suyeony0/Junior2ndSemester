...
public class MyCatViewServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i = 0; i < myCatList.size(); i++) {
			...
			if(temp.equals(""))
			{
				pathList.add(path + "noImage.png");
				System.out.println(path + "noImage.png");
			}
			else {
				pathList.add(path + iDao.getRealName(myCatList.get(i).getId()));
				System.out.println(path + iDao.getRealName(myCatList.get(i).getId()));
			}
		}
